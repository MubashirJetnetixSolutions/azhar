...
export default function InvoicesPage() {
  ...
  const [invoiceType, setInvoiceType] = useState({ value: "Corporate", label: "Corporate" });
  ...
  // The row's originalIndex maps back to the seed `invoices` dataset, which
  // the API uses to pick the SMEs or Corporate invoice template server-side.
  const handleDownload = async (row) => {
    if (downloadingRow) return;
    setDownloadingRow(row.originalIndex);
    try {
      const res = await fetch(`/api/invoices/${row.originalIndex}/pdf`);
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `Invoice ${row.invoiceNo ?? row.originalIndex} - ${row.company}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      setToast({ type: "success", message: "Invoice downloaded successfully.", id: `dl-${++toastSeq.current}` });
    } catch (err) {
      setToast({
        type: "error",
        message: err?.message || "Failed to generate the invoice PDF.",
                    ...
                    <td className="pl-[16px] pr-[24px] py-[12px] h-[60px] align-middle">
                      <div className="flex items-center gap-[6px]">
                        ...
                        <button
                          ...
                          disabled={downloadingRow === row.originalIndex}
                        ...
                        >
                          {downloadingRow === row.originalIndex ? (

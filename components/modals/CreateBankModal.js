...
export default function CreateBankModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* max-h + flex column: header/footer stay pinned while only the body
          scrolls once appended department rows outgrow the viewport. */}
        ...
        <div className="flex-1 min-h-0 px-6 py-5 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <AppSelect
              ...
              size="lg"
              value={creationType}
              onChange={(opt) => { setCreationType(opt); setDate(null); setInvoiceToHead(false); }}
          ...
          {/* Defaulter — shown for both Bank and Branch */}
          <div className="flex items-center justify-between pt-1">
            <p className="text-[13px] text-white">Defaulter</p>
            <Toggle checked={defaulter} onChange={setDefaulter} />
          </div>
          {/* Invoice to Head office — only meaningful when creating a Branch */}
          {creationType?.value === "Branch" && (
            <div className="flex items-center justify-between pt-1">
              <div>
